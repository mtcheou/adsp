											...
											strtSBBHeader mainHeader)
{
    ...
    for (i=0; i < m_iNumElement; i++)
    {
    	if (m_linkStrBook[i].numBlock==1)
		{
		    ...
		    expParm.innerProd = m_linkStrBook[i].coefFirst;
    	...
    	else
		{
			...
			for (j=0; j<m_linkStrBook[i].numBlock; j++ )
			{
				...
				for (k=0; k<expDic.getSignalSize();k++)
				{
					...
					cgSignal[0][ind] = expParm.innerProd*rAtomBlock[k];	
				}
			...
			for (j=0; j<m_linkStrBook[i].numBlock-1; j++ )
			{
		        ...
		        if (fabs(nextrho-expParm.rho) > 0.01*fabs(expParm.rho))
		        {
					...
					expParm.innerProd = cgSignal.dprodInterval(	rLinkAtom,
									...
									strtSBBHeader mainHeader)
{
    ...
    { 
        ...
        {
            ((CExpParm*)parm)->xi = pLinkStrExp[i].xi;
            ((CExpParm*)parm)->phase = pLinkStrExp[i].phase;
            for (j=0; j<pLinkStrExp[i].numBlock; j++ )
            {
                //cout << "   Sintetiza  bloco: "<< j << endl;
                ((CExpParm*)parm)->rho = (double)(pLinkStrExp[i].pRho[j]*
                                                  static_cast<double>(pLinkStrExp[i].pRhoSign[j]));
                if (j==0)
                {
                    ((CExpParm*)parm)->innerProd = pLinkStrExp[i].coefFirst;
                    ((CExpParm*)parm)->a = pLinkStrExp[i].initSample;
                    if (pLinkStrExp[i].numBlock==1)
                        ((CExpParm*)parm)->b = pLinkStrExp[i].endSample;
                    else
                        ((CExpParm*)parm)->b = expDic.getSignalSize()-1;
                }
                ...
                for (k=0; k<expDic.getSignalSize();k++)
                {
                    ...
                    recSignal[index] = recSignal[index] + norm*((CExpParm*)parm)->innerProd*realAtom[k];
                                  ...
                                  int nElement)
{
    ...
    {
        ((CExpParm*)parm)->xi = pLinkStrExp[nElement].xi;
        ((CExpParm*)parm)->phase = pLinkStrExp[nElement].phase;
        for (j=0; j<pLinkStrExp[nElement].numBlock; j++ )
        {
            //cout << "   Sintetiza  bloco: "<< j << endl;
            ((CExpParm*)parm)->rho = (double)(pLinkStrExp[nElement].pRho[j]*
                                           static_cast<double>(pLinkStrExp[nElement].pRhoSign[j]));
            if (j==0)
            {
                ((CExpParm*)parm)->innerProd = pLinkStrExp[nElement].coefFirst;
                ((CExpParm*)parm)->a = pLinkStrExp[nElement].initSample;
                if (pLinkStrExp[nElement].numBlock==1)
                    ((CExpParm*)parm)->b = pLinkStrExp[nElement].endSample;
                else
                    ((CExpParm*)parm)->b = expDic.getSignalSize()-1;
            }
